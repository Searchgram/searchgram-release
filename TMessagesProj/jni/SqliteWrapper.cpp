...
extern "C" {
JNIEXPORT jstring Java_org_telegram_SQLite_SQLitePreparedStatement_sql(JNIEnv *env, jobject object, jlong statementHandle) {
    sqlite3_stmt *handle = (sqlite3_stmt *) (intptr_t) statementHandle;

    const char *str = sqlite3_expanded_sql(handle);
    if (str != 0) {
