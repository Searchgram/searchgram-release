...
public class MessagesStorage extends BaseController {
    ...
    private final static int LAST_FTS_VERSION = 1;
            ...
            { // FTS tables
                ...
                if (version < LAST_FTS_VERSION) {
                    try {
                        ...
                        database.executeFast("CREATE TABLE messages_text(mid INTEGER PRIMARY KEY, uid INTEGER, date INTEGER, message TEXT, web_title TEXT, web_desc TEXT)")
                                .stepThis().dispose();
                        database.executeFast(
                                "CREATE VIRTUAL TABLE messages_fts_v1  USING fts5(uid UNINDEXED, message, web_title, web_desc, content=messages_text, content_rowid=mid, tokenize='trigram bi 1')")
                                .stepThis().dispose();
                        // triggers on messages
                        database.executeFast("CREATE TRIGGER tg_messages_d AFTER DELETE ON messages BEGIN\n"
                                             + "  DELETE FROM messages_text WHERE mid=old.mid; \n"
                                             + "END")
                                .stepThis().dispose();
                        // triggers on messages_text
                        database.executeFast("CREATE TRIGGER tg_messages_text_i AFTER INSERT ON messages_text BEGIN\n"
                                             + "  INSERT INTO messages_fts_v1 (rowid, uid, message, web_title, web_desc) VALUES (new.mid, new.uid, new.message, new.web_title, new.web_desc);\n"
                                             + "END;")
                                .stepThis().dispose();
                        database.executeFast("CREATE TRIGGER tg_messages_text_d AFTER DELETE ON messages_text BEGIN\n"
                                             + "  INSERT INTO messages_fts_v1 (messages_fts_v1, rowid, uid, message, web_title, web_desc) VALUES ('delete', old.mid, old.uid, old.message, old.web_title, old.web_desc);\n"
                                             + "END;")
                                .stepThis().dispose();
                        database.executeFast("CREATE TRIGGER tg_messages_text_u AFTER UPDATE ON messages_text BEGIN\n"
                                             + "  INSERT INTO messages_fts_v1 (messages_fts_v1, rowid, uid, message) VALUES ('delete', old.mid, old.uid, old.message);\n"
                                             + "  INSERT INTO messages_fts_v1 (rowid, uid, message, web_title, web_desc) VALUES (new.mid, new.uid, new.message, new.web_title, new.web_desc);\n"
                                             + "END;")
                                .stepThis().dispose();
                        ...
                        {// XXX: temp way to rebuild indices
                            SQLiteCursor cursor = getDatabase().queryFinalized(
                                    "SELECT mid, uid, date, data from messages");
                            int count = 0;
    ...
    @WorkerThread
    public void ftsSearch(String query, int minDate, int maxDate, int limit, Iterable<Long> usedMids, ArrayList<MessageObject> results) {
        if (TextUtils.isEmpty(query))
            return;

        final long start = System.currentTimeMillis();
        try {
            String inClause = TextUtils.join(",", usedMids);
...
//                            "select mid, uid, date, data from messages where mid NOT IN ("+inClause+") AND mid in (select rowid from messages_fts_v1 where messages_fts_v1 MATCH 'message:\"' || ? || '\"') AND uid=? AND date>=? AND date<=? ORDER BY date DESC LIMIT ?",
            cursor = getDatabase().queryFinalized(
                    "select mid, uid, date, data from messages where mid NOT IN (" + inClause + ") AND mid in (select rowid from messages_fts_v1 where messages_fts_v1 MATCH ?) AND date>=? AND date<=? ORDER BY date DESC LIMIT ?",
...
//                            FileLog.d(String.format("result #%d, mid %d, %s", messages.size(), MessageObject.getIdWithChannel(message), message.message));
    ...
    @WorkerThread
    public void ftsSearchInChat(@Nullable String query, long dialogId, int minDate, int maxDate, int limit, @Nullable Iterable<Long> usedMids, @NonNull Tuple<Integer, ArrayList<MessageObject>, Void> results) {
        if (TextUtils.isEmpty(query))
            return;
        ...
        try {
            ...
            String inClause = usedMids == null ? "" : TextUtils.join(",", usedMids);
            if (inClause.length() == 0)
                inClause = "0";
            ...
            totalCount = getDatabase().executeInt(
                    "select count(1) from messages where mid NOT IN (" + inClause + ") AND mid in (select rowid from messages_fts_v1 where messages_fts_v1 MATCH ?) AND uid=? AND date>=? AND date<=?",
            ...
            if (totalCount > 0) {
                SQLiteCursor cursor;
                cursor = getDatabase().queryFinalized(
                        "select mid, uid, date, data from messages where mid NOT IN (" + inClause + ") AND mid in (select rowid from messages_fts_v1 where messages_fts_v1 MATCH ?) AND uid=? AND date>=? AND date<=? ORDER BY date DESC LIMIT ?",
...
//                            FileLog.d(String.format("result #%d, mid %d, %s", messages.size(), MessageObject.getIdWithChannel(message), message.message));
    ...
    public static CharSequence highlightSearchName(int found, String firstName, String lastName, String username, String q) {
        if (found == 1) {
            ...
            return AndroidUtilities.highlightText(wholeName, q);
        }

        if (found == 2) {
            return AndroidUtilities.highlightText("@"+username, q);
        }
                    ...
                    for (String q : search) {
                        ...
                        if (found != 0) {
                            ...
                            if (data != null) {
                                ...
                                dialogSearchResult.name = highlightSearchName(found, user.first_name, user.last_name, user.username, q);
                                dialogSearchResult.object = user;
                                resultCount++;
                            }
                            break;
                        ...
                        if (matchNameVariants(nameVariants, q) != 0) {
                            ...
                            if (data != null) {
                                ...
                                if (!(chat == null || chat.deactivated || ChatObject.isChannel(chat) && ChatObject.isNotInChat(chat))) {
                                    ...
                                    dialogSearchResult.name = highlightSearchName(1, chat.title, null, null, q);
                    ...
                    for (String q : search) {
                        ...
                        if (found != 0) {
                            ...
                            if (data != null) {
                                ...
                                resultArrayNames.add(highlightSearchName(found, user.first_name, user.last_name, user.username, q));
