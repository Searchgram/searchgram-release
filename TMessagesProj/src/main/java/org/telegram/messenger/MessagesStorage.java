...
public class MessagesStorage extends BaseController {
    ...
    private final MessagesStorageFutures futures = new MessagesStorageFutures(this);
    ...
    private final static int LAST_FTS_VERSION = 2; // upgrade to 2 as messages table has been migrated to messages_v2
	private boolean databaseMigrationInProgress;

    private static final String MESSAGES_TEXT_REPLACE = "REPLACE INTO messages_text (mid, uid, date, message, web_title, web_desc) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public MessagesStorageFutures getFutures() {
        return futures;
    }
    ...
    public void openDatabase(int openTries) {
        ...
        if (BuildVars.LOGS_ENABLED)
            FileLog.d("openTries " + openTries + ", createTable " + createTable + ", currentAccount " + currentAccount + ", cacheFile " + cacheFile);
        ...
        try {
            ...
            { // FTS tables
                Integer version = database.executeInt("SELECT value FROM keyvalue WHERE id='fts_version'");
                if (version == null) version = 0;
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.d("current fts version = " + version);
                }
                if (version < LAST_FTS_VERSION) {
                    try {
                        database.executeFast("DROP TABLE IF EXISTS messages_text")
                                .stepThis().dispose();
                        database.executeFast("DROP TABLE IF EXISTS messages_fts_v1")
                                .stepThis().dispose();
                        database.executeFast("DROP TRIGGER IF EXISTS tg_messages_text_i")
                                .stepThis().dispose();
                        database.executeFast("DROP TRIGGER IF EXISTS tg_messages_text_d")
                                .stepThis().dispose();
                        database.executeFast("DROP TRIGGER IF EXISTS tg_messages_text_u")
                                .stepThis().dispose();
                        database.executeFast("DROP TRIGGER IF EXISTS tg_messages_d")
                                .stepThis().dispose();
                        // messages table saves a chat message in its binary serialization. We need another table for the text content.
                        database.executeFast("CREATE TABLE messages_text(mid INTEGER, uid INTEGER, date INTEGER, message TEXT, web_title TEXT, web_desc TEXT, PRIMARY KEY(mid, uid))") // update primary key as messages_v2
                                .stepThis().dispose();
                        database.executeFast(
                                "CREATE VIRTUAL TABLE messages_fts_v1  USING fts5(mid UNINDEXED, uid UNINDEXED, message, web_title, web_desc, content=messages_text, tokenize='trigram bi 1')") // removed the explicit content_rowid setting which makes fts use the default hidden rowid
                                .stepThis().dispose();
                        // triggers on messages_v2
                        database.executeFast("CREATE TRIGGER tg_messages_d AFTER DELETE ON messages_v2 BEGIN\n"
                                             + "  DELETE FROM messages_text WHERE mid=old.mid AND uid=old.uid; \n"
                                             + "END")
                                .stepThis().dispose();
                        // triggers on messages_text
                        database.executeFast("CREATE TRIGGER tg_messages_text_i AFTER INSERT ON messages_text BEGIN\n"
                                             + "  INSERT INTO messages_fts_v1 (rowid, mid, uid, message, web_title, web_desc) VALUES (new.rowid, new.mid, new.uid, new.message, new.web_title, new.web_desc);\n"
                                             + "END;")
                                .stepThis().dispose();
                        database.executeFast("CREATE TRIGGER tg_messages_text_d AFTER DELETE ON messages_text BEGIN\n"
                                             + "  INSERT INTO messages_fts_v1 (messages_fts_v1, rowid, mid, uid, message, web_title, web_desc) VALUES ('delete', old.rowid, old.mid, old.uid, old.message, old.web_title, old.web_desc);\n"
                                             + "END;")
                                .stepThis().dispose();
                        database.executeFast("CREATE TRIGGER tg_messages_text_u AFTER UPDATE ON messages_text BEGIN\n"
                                             + "  INSERT INTO messages_fts_v1 (messages_fts_v1, rowid, mid, uid, message) VALUES ('delete', old.rowid, old.mid, old.uid, old.message);\n"
                                             + "  INSERT INTO messages_fts_v1 (rowid, mid, uid, message, web_title, web_desc) VALUES (new.rowid, new.mid, new.uid, new.message, new.web_title, new.web_desc);\n"
                                             + "END;")
                                .stepThis().dispose();

                        {// XXX: temp way to rebuild indices
                            SQLiteCursor cursor = getDatabase().queryFinalized(
                                    "SELECT mid, uid, date, data from messages_v2");
                            int count = 0;
                            Tuple<String, String, String> tuple = Tuple.triple("", "", "");
                            ...
                            while (cursor.next()) {
                                ...
                                if (data != null) {
                                    ...
                                    data.reuse();
                                    if (message != null) {
                                        collectText(message, tuple);
                                        if (!TextUtils.isEmpty(tuple._0) || !TextUtils.isEmpty(tuple._1) || !TextUtils.isEmpty(tuple._2)) {
                                            count++;

                                            getDatabase().executeFast(MESSAGES_TEXT_REPLACE)
                                                    .query(mid, uid, date, tuple._0, tuple._1, tuple._2).nextThis()
                                                    .dispose();
                                        }
                                    }
                                }
                            ...
                            if (BuildVars.LOGS_ENABLED)
                                FileLog.d("fts size(rebuild): " + count);
                            Analytics.ftsRebuildSize(currentAccount, count);
                        }

                        database.executeFast("REPLACE INTO keyvalue (id, value) VALUES ('fts_version', ?)").query(LAST_FTS_VERSION).nextThis().dispose();
                        version = LAST_FTS_VERSION;
                    } catch (SQLiteException e) {
                        if (BuildVars.LOGS_ENABLED)
                            FileLog.e("failed to create fts tables for version " + LAST_FTS_VERSION + ": " + e.getMessage(), e);
                    }
                } else {
                    try {
                        int count = database.executeInt("SELECT COUNT(1) FROM messages_text");
                        if (BuildVars.LOGS_ENABLED)
                            FileLog.d("fts size: " + count);
                        Analytics.ftsSize(currentAccount, count);
                    } catch (SQLiteException e) {
                        if (BuildVars.LOGS_ENABLED)
                            FileLog.e("failed to check fts size: " + e.getMessage(), e);
                    }
                }
            }
        } catch (Exception e) {
    ...
    private static void collectText(@NonNull TLRPC.Message message, @NonNull Tuple<String, String, String> result) {
        String messageText = message.message;
        String webTitle = "";
        String webDesc = "";
        if (message.media instanceof TLRPC.TL_messageMediaWebPage && message.media.webpage instanceof TLRPC.TL_webPage) {
            TLRPC.WebPage webPage = message.media.webpage;
            webTitle = webPage.title;
            if (webTitle == null)
                webTitle = webPage.site_name;
            webDesc = webPage.description;
        }
        result._0 = messageText;
        result._1 = webTitle;
        result._2 = webDesc;
    }
        ...
        storageQueue.postRunnable(() -> {
            try {
                ...
                if (!messages.isEmpty()) {
                    ...
                    SQLitePreparedStatement stText = database.executeFast(MESSAGES_TEXT_REPLACE);
                    Tuple<String, String, String> tuple = Tuple.triple("", "", "");
                    for (int a = 0; a < messages.size(); a++) {
                        ...
                        stText.bindInteger(1, message.id);
                        state.bindLong(2, message.dialog_id);
                        stText.bindLong(2, message.dialog_id);
                        ...
                        stText.bindInteger(3, message.date);
                        state.bindByteBuffer(6, data);
                        stText.bindString(4, tuple._0, true);
                        stText.bindString(5, tuple._1, true);
                        stText.bindString(6, tuple._2, true);
        ...
        try {
            if (scheduled) {
                ...
                for (int a = 0; a < messages.size(); a++) {
                    ...
                    state_messages.bindInteger(1, messageId);
                    stText.bindInteger(1, messageId);
    ...
    @WorkerThread
    public void ftsSearch(String query, int minDate, int maxDate, int limit, Iterable<MessageId> usedMids, ArrayList<MessageObject> results) {
        if (TextUtils.isEmpty(query))
            return;

        final long start = System.currentTimeMillis();
        try {
            String inClause = TextUtils.join(",", usedMids); // (mid, uid), (mid, uid), ...
            if (inClause.length() == 0)
                inClause = "(0,0)";

            SQLiteCursor cursor;
//                            "select mid, uid, date, data from messages where mid NOT IN ("+inClause+") AND mid in (select rowid from messages_fts_v1 where messages_fts_v1 MATCH 'message:\"' || ? || '\"') AND uid=? AND date>=? AND date<=? ORDER BY date DESC LIMIT ?",
            cursor = getDatabase().queryFinalized(
                    "select mid, uid, date, data from messages_v2 where (mid, uid) NOT IN (VALUES " + inClause + ") AND (mid, uid) IN (select mid, uid from messages_fts_v1 where messages_fts_v1 MATCH ?) AND date>=? AND date<=? ORDER BY date DESC LIMIT ?",
                    query, minDate, maxDate, limit);
            ...
            while (cursor.next()) {
                ...
                if (data != null) {
                    ...
                    data.reuse();
                    if (message != null) {
                        //XXX: use mid and date from messages table?
                        //XXX: reply messages?
                        message.dialog_id = uid;
//                            FileLog.d(String.format("result #%d, mid %d, %s", messages.size(), MessageObject.getIdWithChannel(message), message.message));
                        MessageObject mo = new MessageObject(currentAccount, message, false, false);
                        mo.setQuery(query);
                        mo.isFtsSearch = true;
                        results.add(mo);
                    }
        ...
        Analytics.ftsGlobalTime(System.currentTimeMillis() - start);
    }

    @WorkerThread
    public void ftsSearchInChat(@Nullable String query, long dialogId, int minDate, int maxDate, int limit, @Nullable Iterable<MessageId> usedMids, @NonNull Tuple<Integer, ArrayList<MessageObject>, Void> results) {
        if (TextUtils.isEmpty(query))
            return;
        ...
        try {
            ...
            String inClause = usedMids == null ? "" : TextUtils.join(",", usedMids); // (mid, uid), (mid, uid), ...
            if (inClause.length() == 0)
                inClause = "(0,0)";

            // Note: the total count is the number in all time range, so it doesn't need to respect time settings
            totalCount = getDatabase().executeInt(
                    "select count(1) from messages_v2 where (mid,uid) NOT IN (VALUES " + inClause + ") AND (mid,uid) IN (SELECT mid, uid FROM messages_fts_v1 WHERE messages_fts_v1 MATCH ?) AND uid=? AND date>=? AND date<=?",
                    query, dialogId, 0, Integer.MAX_VALUE);

            results._0 = totalCount;

            if (totalCount > 0) {
                SQLiteCursor cursor;
                cursor = getDatabase().queryFinalized(
                        "select mid, uid, date, data from messages_v2 where (mid,uid) NOT IN (VALUES " + inClause + ") AND (mid,uid) in (SELECT mid, uid FROM messages_fts_v1 WHERE messages_fts_v1 MATCH ?) AND uid=? AND date>=? AND date<=? ORDER BY date DESC LIMIT ?",
                        query, dialogId, minDate, maxDate, limit);
...
//                            FileLog.d(String.format("result #%d, mid %d, %s", messages.size(), MessageObject.getIdWithChannel(message), message.message));
        ...
        Analytics.ftsChatTime(System.currentTimeMillis() - start);
    }

    private static boolean matchName(String name, String q) {
        return name != null && name.contains(q);
    }

    private static int matchNameVariants(Tuple<String, String, String> v, String q) {
        if (matchName(v._0, q) || matchName(v._1, q))
            return 1;
        if (matchName(v._2, q))
            return 2;
        return 0;
    ...
    private static void parseName(@NonNull String name, @NonNull Tuple<String, String, String> v) {
        v._2 = null;
        int pos = name.lastIndexOf(";;;");
        if (pos != -1) {
            v._2 = name.substring(pos + 3);
        }

        v._0 = name;
        pos = name.indexOf(";;;");
        if (pos != -1)
            v._0 = name.substring(0, pos);

        v._1 = LocaleController.getInstance().getTranslitString(v._0);
        if (v._0.equals(v._1)) {
            v._1 = null;
        }
    }

    public static CharSequence highlightSearchName(int found, String firstName, String lastName, String username, String q, Theme.ResourcesProvider resourcesProvider) {
        if (found == 1) {
            ...
            return AndroidUtilities.highlightText(wholeName, q, resourcesProvider);
        }

        if (found == 2) {
            return AndroidUtilities.highlightText("@"+username, q, resourcesProvider);
        }
    ...
    // search for dialogs
    @WorkerThread
    public void localSearch(int dialogsType, String query, ArrayList<Object> resultArray, ArrayList<CharSequence> resultArrayNames, ArrayList<TLRPC.User> encUsers, int folderId/*, Theme.ResourcesProvider resourcesProvider*/) {
        ...
        try {
            ...
            Tuple<String, String, String> nameVariants = Tuple.triple("", null, null);

            if (!usersToLoad.isEmpty()) { // search in users having dialogs
                ...
                while (cursor.next()) {
                    ...
                    parseName(name, nameVariants);

                    for (String q : search) {
                        int found = matchNameVariants(nameVariants, q);
                        if (found != 0) {
                            NativeByteBuffer data = cursor.byteBufferValue(0);
                            if (data != null) {
                                TLRPC.User user = TLRPC.User.TLdeserialize(data, data.readInt32(false), false);
                                ...
                                dialogSearchResult.name = highlightSearchName(found, user.first_name, user.last_name, user.username, q, null);
                                dialogSearchResult.object = user;
                                resultCount++;
                            }
                            break;
            ...
            if (!chatsToLoad.isEmpty()) {
                ...
                while (cursor.next()) {
                    ...
                    parseName(name, nameVariants);
                    for (String q : search) {
                        if (matchNameVariants(nameVariants, q) != 0) {
                            NativeByteBuffer data = cursor.byteBufferValue(0);
                            if (data != null) {
                                TLRPC.Chat chat = TLRPC.Chat.TLdeserialize(data, data.readInt32(false), false);
                                data.reuse();
                                if (!(chat == null || chat.deactivated || ChatObject.isChannel(chat) && ChatObject.isNotInChat(chat))) {
                                    ...
                                    dialogSearchResult.name = highlightSearchName(1, chat.title, null, null, q, null);
            ...
            if (!encryptedToLoad.isEmpty()) { // search in enc_chats
            ...
            if (dialogsType != 2) { // search in contacts
                    ...
                    for (String q : search) {
                        ...
                        if (found != 0) {
                            ...
                            if (data != null) {
                                ...
                                resultArrayNames.add(highlightSearchName(found, user.first_name, user.last_name, user.username, q, null));
        ...
        if (BuildVars.LOGS_ENABLED)
            FileLog.d(String.format("localSearch results: dialogsType %d, query %s, resultArray %s, resultArrayNames %s, encUsers %s, folderId %d", dialogsType, query, resultArray, resultArrayNames, encUsers, folderId));
    }
}
