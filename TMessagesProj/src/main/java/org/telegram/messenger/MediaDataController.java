...
import org.telegram.tgnet.ConnectionsManagerFutures;
import org.telegram.tgnet.NativeByteBuffer;
import org.telegram.tgnet.SerializedData;
import org.telegram.tgnet.TLObject;
import org.telegram.tgnet.TLRPC;
import org.telegram.ui.ActionBar.BaseFragment;
import org.telegram.ui.Adapters.SearchMessagesSession;
...
//    private int reqId;
    ...
    private boolean[] messagesSearchEndReached = new boolean[]{false, false}; // index 1 is for mergedDialogId which is rare
    ...
    private @Nullable CompletableFuture<Void> loadingFuture;
    private boolean ftsReachesEnd = false;
    private int serverCount = 0;
    private boolean serverReachesEnd = false;
    private ArrayList<MessageObject> pendingMessageObjects = new ArrayList<>();
    ...
    public void searchMessagesInChat(String query, final long dialogId, final long mergeDialogId, final int guid, final int direction, int replyMessageId, TLRPC.User user, TLRPC.Chat chat, boolean isEncryptedChat) {
        searchMessagesInChat(query, dialogId, mergeDialogId, guid, direction, replyMessageId, false, user, chat, true, isEncryptedChat);
    }
    ...
    public void loadMoreSearchMessages(boolean isEncryptedChat) {
        ...
        searchMessagesInChat(null, lastDialogId, lastMergeDialogId, lastGuid, 1, lastReplyMessageId, false, lastSearchUser, lastSearchChat, false, isEncryptedChat);
        lastReturnedNum = temp;
        loadingMoreSearchMessages = true;
    }

    private void searchMessagesInChat(String query, final long dialogId, final long mergeDialogId, final int guid, final int direction, int replyMessageId, final boolean internal, final TLRPC.User user, final TLRPC.Chat chat, boolean jumpToMessage, boolean isEncryptedChat) {
        if (BuildVars.LOGS_ENABLED)
            FileLog.d(String.format("query %s, dialogId %s, mergeDialogId %s, guid %s, direction %s, replyMessageId %s, internal %s, user %s, chat %s, jumpToMessage %s",
                                    query, dialogId, mergeDialogId, guid, direction, replyMessageId, internal, user, chat, jumpToMessage));
        int max_id = 0;
        long queryWithDialog = dialogId;
        // `internal` is true when this method is called recursively from itself.
        ...
        if (query == null) { // jump to next/previous search results in bubbles
            if (searchResultMessages.isEmpty()) {
                return;
            }
            if (direction == 1) { // to an older result
                lastReturnedNum++;
            ...
            } else if (direction == 2) { // to a newed result
                lastReturnedNum--;
                if (lastReturnedNum < 0) {
                    lastReturnedNum = 0;
                    return;
            ...
            ftsReachesEnd = false;
            serverReachesEnd = isEncryptedChat;
            pendingMessageObjects.clear();
            serverCount = 0;
            if (isEncryptedChat)
                Analytics.searchEvent(Analytics.SEARCH_SCENE_SECURE, query);
            else
                Analytics.searchEvent(Analytics.SEARCH_SCENE_CHAT, query);
        }
        if (messagesSearchEndReached[0] && !messagesSearchEndReached[1] && mergeDialogId != 0) {
            queryWithDialog = mergeDialogId;
        }
        if (queryWithDialog == dialogId && firstQuery) {
            if (mergeDialogId != 0 && !isEncryptedChat) {
                ...
                mergeReqId = getConnectionsManager().sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() -> {
                    if (lastMergeDialogId == mergeDialogId) {
                        ...
                        if (response != null) {
                            ...
                            searchMessagesInChat(req.q, dialogId, mergeDialogId, guid, direction, replyMessageId, true, user, chat, jumpToMessage, isEncryptedChat);
                        } else {
        ...
        ConnectionsManagerFutures.RequestFuture<TLObject, ArrayList<MessageObject>> serverFuture;
        if (serverReachesEnd || isEncryptedChat) {
            serverFuture = ConnectionsManagerFutures.RequestFuture.completedFuture(null, new ArrayList<>());
        } else {
            serverCount = 0;
            TLRPC.TL_messages_search req = new TLRPC.TL_messages_search();
            ...
            serverFuture =
                    ConnectionsManager.getInstance(currentAccount).getFutures().sendRequest(req, response -> {
        ...
        loadingFuture = serverFuture.thenCompose(responseObject -> {
            Futures.UIThreadFuture<Tuple<Integer, ArrayList<MessageObject>, Void>> ftsFuture;
            if (ftsReachesEnd || queryWithDialogFinal != dialogId)
                ftsFuture = Futures.UIThreadFuture.completedFuture(Tuple.<Integer, ArrayList<MessageObject>>pair(0, new ArrayList<>()));
            else {
                Set<MessageId> seenMids = new HashSet<>();

                for (MessageObject o : searchResultMessages) {
                    seenMids.add(MessageId.of(o.getId(), o.getDialogId())); // replace getIdWithChannel()
                }

                for (MessageObject o : pendingMessageObjects) {
                    seenMids.add(MessageId.of(o.getId(), o.getDialogId())); // replace getIdWithChannel()
                }

                ArrayList<MessageObject> serverMessageObjects = ArrayUtils.nonNull(responseObject.data);
                for (MessageObject o : serverMessageObjects) {
                    seenMids.add(MessageId.of(o.getId(), o.getDialogId())); // replace getIdWithChannel()
                }

                int minDate = 0;
                if (!serverMessageObjects.isEmpty())
                    minDate = serverMessageObjects.get(serverMessageObjects.size() - 1).messageOwner.date;
                ftsFuture = MessagesStorage.getInstance(currentAccount).getFutures().ftsSearchInChat(finalQuery, dialogId, minDate, Integer.MAX_VALUE, 20, seenMids);
            }

            return ftsFuture.thenApply(ftsTuple -> Tuple.triple(responseObject, ftsTuple._0, ftsTuple._1));
        }).thenAccept(tuple -> {
            ...
            @Nullable TLRPC.messages_Messages res = (TLRPC.messages_Messages) tuple._0.res;
            if (res != null) { // update users, chats from server response
            ...
            if (finalOffsetId == 0 && queryWithDialogFinal == dialogId) {
                lastReturnedNum = 0;
                searchResultMessages.clear();
                searchResultMessagesMap[0].clear();
                searchResultMessagesMap[1].clear();
            ...
            ArrayList<MessageObject> serverMessageObjects = ArrayUtils.nonNull(tuple._0.data);
            int ftsTotalCount = tuple._1;
            ArrayList<MessageObject> ftsMessageObjects = ArrayUtils.nonNull(tuple._2);

            pendingMessageObjects.addAll(serverMessageObjects);
            pendingMessageObjects.addAll(ftsMessageObjects);
            SearchMessagesSession.sort(pendingMessageObjects);

            boolean added = false;
            { // move objects from pending to results
                List<MessageObject> topMessageObjects = pendingMessageObjects.subList(0, Math.min(20, pendingMessageObjects.size()));
                for (int a = 0; a < topMessageObjects.size(); a++) {
                    added = true;
                    MessageObject messageObject = topMessageObjects.get(a);
                ...
                topMessageObjects.clear(); // which clears objects in pendingMessageObjects
            }

            if (queryWithDialogFinal != dialogId) {
                if (res != null) {
                    messagesSearchEndReached[1] = res.messages.size() < 21;
                    messagesSearchCount[1] = res instanceof TLRPC.TL_messages_messagesSlice || res instanceof TLRPC.TL_messages_channelMessages ? res.count : res.messages.size();
                }
            } else {
                if (res != null) {
                    serverReachesEnd = res.messages.size() < 21;
                    serverCount = res instanceof TLRPC.TL_messages_messagesSlice || res instanceof TLRPC.TL_messages_channelMessages ? res.count : res.messages.size();
                } else {
                    serverReachesEnd = true;
                }

                ftsReachesEnd = serverReachesEnd && ftsMessageObjects.size() < 20;
                messagesSearchEndReached[0] = serverReachesEnd && ftsReachesEnd && pendingMessageObjects.isEmpty();
                messagesSearchCount[0] = serverCount + ftsTotalCount;
            }
            ...
            if (queryWithDialogFinal == dialogId && messagesSearchEndReached[0] && mergeDialogId != 0 && !messagesSearchEndReached[1]) {
                searchMessagesInChat(lastSearchQuery, dialogId, mergeDialogId, guid, 0, replyMessageId, true, user, chat, jumpToMessage, isEncryptedChat);
            }
        });
    }
