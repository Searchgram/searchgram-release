...
public class DialogsSearchAdapter extends RecyclerListView.SelectionAdapter {
    ...
    private SearchMessagesSession _searchMessagesSession = SearchMessagesSession.EMPTY_SESSION;
...
//    private int lastReqId;
    ...
    private boolean messagesSearchEndReached; // for UI
...
//    private int nextSearchRate;
    ...
    public void loadMoreSearchMessages() {
        if (_searchMessagesSession.isLoading())
            return;

        searchMessagesInternal(_searchMessagesSession.query, lastMessagesSearchId);
    }

    public String getLastSearchString() {
        return _searchMessagesSession.query;
    }

    @MainThread
    private void searchMessagesInternal(final String query, int searchId) {
        if (needMessagesSearch == 0 || TextUtils.isEmpty(_searchMessagesSession.query) && TextUtils.isEmpty(query)) {
            return;
        }

        _searchMessagesSession.cancelLoading();

        if (TextUtils.isEmpty(query)) {
            updateSession(SearchMessagesSession.EMPTY_SESSION);
//            lastReqId = 0;
//            lastMessagesSearchString = null;
        ...
        if (!query.equals(_searchMessagesSession.query)) {
            updateSession(new SearchMessagesSession(currentAccount, query, 20, this.folderId));
        }

//        final int currentReqId = ++lastReqId;
        SearchMessagesSession session = _searchMessagesSession;
        session.loadMore().thenAccept(__ -> {
            if (session != _searchMessagesSession) {
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.w("different search session: " + session.debug() + ", current "
                              + _searchMessagesSession.debug());
                }
                return;
            }
            waitingResponseCount--;

            searchResultMessages = _searchMessagesSession.copyResultMessages();
            searchWas = true;
            messagesSearchEndReached = _searchMessagesSession.noMoreData();
            if (searchId > 0) {
            ...
            if (BuildVars.LOGS_ENABLED) {
                FileLog.d("notify searchResultMessages " + searchResultMessages.size());
            }
            notifyDataSetChanged();
            if (delegate != null) {
                delegate.searchStateChanged(waitingResponseCount > 0, true);
    ...
    private void updateSession(SearchMessagesSession session) {
        if (_searchMessagesSession == session)
            return;

        // FileLog.w("updating session from " + searchMessagesWithFtsSession.debug() + ", to " + session.debug());
        _searchMessagesSession.cancelLoading();
        _searchMessagesSession = session;
        Analytics.searchEvent(Analytics.SEARCH_SCENE_GLOBAL, session.query);
    }
